...
class Coach:
	...
	def trainEpoch(self):
		...
		for i, batch in enumerate(diffusionLoader):
			...
			diff_loss, ckd_loss = self.diffusion_model.training_losses(self.denoise_model, remaindItem, ui_matrix, uEmbeds, iEmbeds, batch_idx, maskedBatch)
			loss = diff_loss.mean() * (1-args.e_loss) + ckd_loss.mean() * args.e_loss

			epDiLoss += diff_loss.mean().item()
			epUKLoss += ckd_loss.mean().item()
		...
		for i, tem in enumerate(trnLoader):
			ancs, poss, negs = tem
			ancs = ancs.long().cuda() #user
			poss = poss.long().cuda() #item
			negs = negs.long().cuda()
		...
		ret['CKDLoss'] = epUKLoss / diffusionLoader.dataset.__len__()
		return ret
