...
class Denoise(nn.Module):
	...
	def forward(self, x, timesteps,con_emb, maskInfo, mess_dropout=True):
		...
		if mess_dropout:
			x = self.drop(x)
		if self.mask_BCE:
			h = torch.cat([x+maskInfo, emb, con_emb], dim=-1)
		else:
			x = x.cuda()
			emb = emb.cuda()
			h = torch.cat([x, emb, con_emb], dim=-1)
...
class GaussianDiffusion(nn.Module):
	...
	def training_losses(self, model, x_start, ui_matrix, userEmbeds, itmEmbeds, batch_idx=None, batchMask=None):
		batch_size = x_start.size(0)
		ts = torch.randint(0, self.steps, (batch_size,)).long().cuda()
		Orin_itmEmbeds = itmEmbeds[batch_idx]
		noise = torch.randn_like(x_start)
		...
		model_output = model(x_t, ts, x_start, batchMask)
		Ui_matrix = torch.spmm(ui_matrix, model_output[:, :args.item].t()).t()
		mse = self.MSE((x_start - model_output) ** 2)
		...
		kg_Embeds = torch.mm(Ui_matrix, userEmbeds)
		ckd_loss = self.MSE((kg_Embeds - Orin_itmEmbeds) ** 2)
		return diff_loss, ckd_loss
